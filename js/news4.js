	...
	setInstance() {
		...
		function tmpl_item (item) {
			return `<tr class="ng-scope">` +
              			`<td class="ng-binding normalFrontIcon">공지</td>` +
...
(function ($, global) {
	...
	nc.lineage.news = function (option) {
		if (isNotDefined(option)) throw new Error('nc.lineage.news require data object');
		...
		getDatas([
			...
			option.data = json_data;
			news = new News(option).init();
		}, (data) => {
		});
	};
