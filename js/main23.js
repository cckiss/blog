includeCSS('https://cdn.jsdelivr.net/gh/cckiss/blog/css/l1.main6.css');
	...
	setInstance() {
		...
		let tmpl = _.option.data.map( (item, i) => {
			return `<section class="section-powerbook section-${item.contentsId}">` +
				`<header><h2><a href="${item.url}">${item.header}</a></h2></header>` +
				`<article>` +
					`<a href="${item.url}">` +
...
const main_tpl = `<section class="section-promotion"></section>` +
	`<div class="wrap-section-shortcut"><section class="section-shortcut"></section></div>` +
	`<div class="wrap-section wrap-section-portal">` +
		`<section class="bundle">` +
		`<section class="section-powerbookcartoon"></section>` +
	`</section>` +
`</div>` +
`<div class="section-blank"></div>` +
`<div class="wrap-section wrap-section-article"></div>`;
$('#container .post-body.entry-content').prepend(main_tpl);
...
(function ($, global) {
	...
	nc.lineage.main = function (main_data) {
		let contiGroupData = {};
		...
		function init() {
			...
			// index 페이지 데이터 로드
			$.each(main_data, function(key, element) {
				switch(key){
				case 'PROMOTIONS':
