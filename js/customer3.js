includeCSS('https://cdn.jsdelivr.net/gh/cckiss/blog/css/customer1.css');
			...
			return `<section class="download">` +
				`<div class="download_area">` +
					`<div class="download_area_inner">` +
						...
						`</div>` +
						`<a href="${item.url}" id="downloadUrl" class="btn_download" title="" target="${item.target}">${item.button}</a>` +
					`</div>` +
				`</div>` +
			`</section>`;
		}).join('');
