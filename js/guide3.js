				...
				`<div class="wrap-section-beginner class-0">` +
					`<div class="wrap-section-top">` +
						`<section class="section-top">` +
							`<header>` +
								`<p class="copy__subtitle">${document.title} 입문자를 위한 필독 지침서</p>` +
								`<span class="copy__x"></span>` +
								`<h1 class="copy__title">${document.title} 용사 가이드</h1>` +
								`<p class="copy__menulist">화폐 ㆍ 사냥터 ㆍ 제작</p>` +
							`</header>` +
						`</section>` +
					`</div>` +
					`<div class="wrap-section-item"><section class="section-item"></section></div>` + 
				`</div>` +
			`</div>` +
		`</div>`;
