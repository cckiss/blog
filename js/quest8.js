...
class Quest {
	...
	setInstance() {
		...
		let tmpl = _.option.data.map( (item, index) => {
			return `<table class="table table-size-1" id="quest-${index}" style="${index == 0 ? `display: table;` : ``}">` +
...
(function ($, global) {
	...
	nc.lineage.quest = function (quest_data) {
		...
		const tpl = `<div class="wrap-section-beginner">` +
			`<div class="wrap-section-top"><section class="section-top"><header><p class="copy__subtitle">${document.title} 퀘스트</p><span class="copy__x"></span><h1 class="copy__title">퀘스트 정보</h1></header></section></div>` +
			`<div class="wrap-section-hunt">` +
				`<section class="section-hunt">` +
					`<article>` +
						`<div class="article-tab">` +
							`<ul class="tab">${quest_data.map( (item, index) => { return `<li class="${index == 0 ? 'on' : ''}"><a href="" data-lv="${index}">${item.class_name}</a></li>`; }).join('')}</ul>` +
						`</div>` +
